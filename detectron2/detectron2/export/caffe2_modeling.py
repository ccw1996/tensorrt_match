...
class Caffe2YOLOF(Caffe2MetaArch):
    def __init__(self, cfg, torch_model):
        assert isinstance(torch_model, meta_arch.YOLOF)
        super().__init__(cfg, torch_model)
    ...
    def forward(self, inputs):
        ...
        features = self._wrapped_model.backbone(images.tensor)
        features = [features[self._wrapped_model.backbone_level]]
        # for i, feature_i in enumerate(features):
        #     #features[i] = alias(feature_i, "feature_{}".format(i), is_backward=False)
        #     features[i] = alias(feature_i, "feature_{}".format(i))
        #     return_tensors.append(features[i])

        # pred_logits, pred_anchor_deltas = self._wrapped_model.decoder(self._wrapped_model.encoder(features[0]))
        # for i, (box_cls_i, box_delta_i) in enumerate(zip(pred_logits, pred_anchor_deltas)):
        #     return_tensors.append(alias(box_cls_i, "box_cls_{}".format(i)))
        #     return_tensors.append(alias(box_delta_i, "box_delta_{}".format(i)))

        output =self._wrapped_model.encoder(features[0])
        return_tensors.append(alias(output, "box_cls_0"))
        return tuple(return_tensors)
    ...
    def encode_additional_info(self, predict_net, init_net):
        ...
        check_set_pb_arg(predict_net, "meta_architecture", "s", b"YOLOF")

        # Inference parameters:
        # check_set_pb_arg(
        #     predict_net, "pos_ignore_thresh", "f", _cast_to_f32(self._wrapped_model.pos_ignore_thresh)
        # )
        # check_set_pb_arg(
        #     predict_net, "neg_ignore_thresh", "f", _cast_to_f32(self._wrapped_model.neg_ignore_thresh)
        # )
        check_set_pb_arg(
            predict_net, "score_threshold", "f", _cast_to_f32(self._wrapped_model.test_score_thresh)
        )
    ...
    def get_outputs_converter(predict_net, init_net):
        ...
        # hack to reuse inference code from RetinaNet
        self.inference = functools.partial(meta_arch.YOLOF.inference, self)
        self.inference_single_image = functools.partial(
            meta_arch.YOLOF.inference_single_image, self
        )
